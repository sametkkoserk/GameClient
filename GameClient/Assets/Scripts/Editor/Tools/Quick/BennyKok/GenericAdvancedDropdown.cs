...
namespace Editor.Tools.Quick.BennyKok
{
  ...
  {
    private readonly Dictionary<string, Action> pathToActionMap = new();
    private readonly Dictionary<int, Action> idToActionMap = new();
    private readonly string title = "Items";

#if UNITY_2021_2_OR_NEWER
    private static readonly PropertyInfo menuItemsField = typeof(GenericMenu).GetProperty("menuItems",
    ...
    private static readonly FieldInfo menuItemContentField = typeof(GenericMenu).GetNestedType("MenuItem", BindingFlags.NonPublic |
                                                                                                           BindingFlags.Instance)?.GetField("content",
      BindingFlags.Public |
      BindingFlags.Instance);

    private static readonly FieldInfo menuItemMenuFunctionField = typeof(GenericMenu).GetNestedType("MenuItem", BindingFlags.NonPublic |
                                                                                                                BindingFlags.Instance)?.GetField("func",
      BindingFlags.Public |
      BindingFlags.Instance);

    private static readonly FieldInfo menuItemMenuFunction2Field = typeof(GenericMenu).GetNestedType("MenuItem", BindingFlags.NonPublic |
                                                                                                                 BindingFlags.Instance)?.GetField("func2",
      BindingFlags.Public |
      BindingFlags.Instance);

    private static readonly FieldInfo menuItemUserDataField = typeof(GenericMenu).GetNestedType("MenuItem", BindingFlags.NonPublic |
                                                                                                            BindingFlags.Instance)?.GetField("userData",
      BindingFlags.Public |
      BindingFlags.Instance);

    private static readonly FieldInfo menuItemSeparatorField = typeof(GenericMenu).GetNestedType("MenuItem", BindingFlags.NonPublic |
    ...
    public void Dropdown(Rect rect, int minLineCount = -1)
    {
      ...
      var size = new Vector2(rect.width, 100);

      if (minLineCount > 0) size.y = minLineCount * EditorGUIUtility.singleLineHeight;
      minimumSize = size;
    ...
    protected override AdvancedDropdownItem BuildRoot()
    {
      ...
      foreach (var item in pathToActionMap)
      {
        ...
        if (lastItem != null && !idToActionMap.ContainsKey(lastItem.id)) idToActionMap.Add(lastItem.id, item.Value);
      }
    ...
    protected override void ItemSelected(AdvancedDropdownItem item)
    {
      ...
      if (idToActionMap.TryGetValue(item.id, out var action)) action();
    }
  }
}