...
namespace StrangeIoC.scripts.strange.framework.impl
{
  public class Binder : IBinder
  {
    ...
    public virtual void ResolveBinding(IBinding binding, object key)
    {
      ...
      if (bindings.ContainsKey(key))
      {
        ...
        if (dict.ContainsKey(bindingName))
        {
          ...
          if (existingBinding.isWeak)
            ...
            if (existingBinding != binding && existingBinding.isWeak && (!binding.isWeak || existingBinding.value == null || existingBinding.value is Type))
      ...
      if (dict.ContainsKey(BindingConst.NULLOID) && dict[BindingConst.NULLOID] == binding) dict.Remove(BindingConst.NULLOID);
