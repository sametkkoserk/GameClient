  ...
  public class Injector : IInjector
  {
    ...
    public object Instantiate(IInjectionBinding binding)
    {
      ...
      if (retv == null) //If we don't have an existing value, go ahead and create one.
      {
        ...
        if (retv != null)
        {
          if (binding.toInject) retv = Inject(retv, false);
    ...
    private void armorAgainstInfiniteLoops(IInjectionBinding binding)
    {
      ...
      if (infinityLock == null) infinityLock = new Dictionary<IInjectionBinding, int>();
      if (infinityLock.ContainsKey(binding) == false) infinityLock.Add(binding, 0);
      infinityLock[binding] = infinityLock[binding] + 1;
      if (infinityLock[binding] > INFINITY_LIMIT) throw new InjectionException("There appears to be a circular dependency. Terminating loop.", InjectionExceptionType.CIRCULAR_DEPENDENCY);
    }
  }
}