...
namespace StrangeIoC.scripts.strange.extensions.pool.impl
{
  ...
  public class Pool : IPool, IPoolable
  {
    ...
    protected Stack instancesAvailable = new();

    /// A HashSet of the objects checked out of the Pool.
    protected HashSet<object> instancesInUse = new();
    ...
    public virtual object GetInstance()
    {
      ...
      if (size > 0)
      {
        ...
        else
        {
          ...
          if (overflowBehavior == PoolOverflowBehavior.WARNING) Console.WriteLine("WARNING: A pool has overflowed its limit.\n\t\tPool type: " + poolType, PoolExceptionType.OVERFLOW);
          return null;
        }
      }
      else
